...
(function () {
  ...
  /* ---------- Exportar PDF (página Produto) ---------- */
  // Sem backend, então usamos o "Imprimir" do navegador com uma folha de
  // estilos de impressão dedicada (@media print) que esconde header/footer
  // e deixa só o conteúdo do produto — no diálogo de impressão, escolher
  // "Salvar como PDF" gera o arquivo.
  const exportPdfBtn = document.getElementById('exportPdfBtn');
  exportPdfBtn?.addEventListener('click', () => window.print());

  /* ---------- Formulário de contato (página Quem Somos) ---------- */
  ...
  function validateContactField(input) {
    ...
    if (input.validity.valueMissing) {
      ...
      if (errorEl) errorEl.textContent = 'Esse campo é obrigatório.';
      return false;
    }
    if (input.type === 'email' && input.validity.typeMismatch) {
      input.classList.add('invalid');
      if (errorEl) errorEl.textContent = 'Digite um e-mail válido.';
      return false;
    }
    if (input.hasAttribute('minlength') && input.value.trim().length < Number(input.getAttribute('minlength'))) {
      input.classList.add('invalid');
      if (errorEl) errorEl.textContent = input.tagName === 'TEXTAREA' ? 'Escreva um pouco mais.' : 'Muito curto.';
      return false;
    }
  ...
  /* ==========================================================
     ...
     2. Crie um novo formulário e copie o "Form ID" que ele te dá
        (algo como "xzzjgqrp")
     3. Cole no lugar de "SEU_FORM_ID" logo abaixo
     Sem isso preenchido, o formulário continua só validando os
     campos e mostrando a mensagem de sucesso, sem enviar nada de
     verdade — é só trocar essa linha que passa a funcionar.
     ========================================================== */
  ...
  contactForm?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      alert('Não foi possível enviar sua mensagem agora. Tente novamente em instantes ou escreva direto para contato@lumasafe.pin.');
    }
  });
})();
